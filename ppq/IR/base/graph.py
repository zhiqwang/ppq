...
class Opset:
    def __init__(self, domain: str = DEFAULT_OPSET_DOMAIN, version: int = DEFAULT_OPSET_VERSION) -> None:
        ...
        empowers AI developers to choose the right tools as their project
        evolves.
...
class Variable(Serializable):
    def __init__(
        ...
        dest_ops: List[OperationBase] = None,
        source_op: OperationBase = None,
    ) -> None:
        super().__init__()
        self._name = name
    ...
    def meta(self) -> TensorMeta:
        if self.source_op is not None:
            if self.source_op.meta_data is None:
                return None
            ...
            if dest_op.meta_data is None:
                return None
            return dest_op.meta_data.input_metas[dest_idx]
        else:
            raise RuntimeError(
                f"Seems you got an isolated variable {self.name}, " "PPQ is not able to infer its meta data yet."
            )
    ...
    def shape(self, new_shape: List[Union[Text, int, None]]):
        if self.meta is None:
            raise PermissionError(
                f"Can not assign shape with variable {self.name}, " "cause it is not linked within a graph."
            )
        if self.is_parameter == True:
            raise PermissionError(
                f"Can not assign shape with variable {self.name}, " "cause it is a parameter varaible."
            )
...
class BaseGraph(Serializable):
    ...
    def delete_operation(self, op_name: str, cascade: bool = False, force_delete: bool = False):
        ...
        if not isinstance(op_name, str):
            raise TypeError(f"This function needs a operation name as parameter, " f"while {type(op_name)} was given")
        if op_name not in self.operations:
            return
        operation = self.operations[op_name]
        if len(operation.outputs) != 0 and not cascade and not force_delete:
            raise PermissionError(
                f"It is not safe to delete operation {op_name}, "
    ...
    def delete_variable(self, var_name: str, force_delete: bool = False):
        ...
        if not isinstance(var_name, str):
            raise TypeError(
                f"This function need a variable name to delete variable from graph, "
        ...
        if len(variable.dest_ops) != 0 and not force_delete:
            raise PermissionError(
                f"It is not safe to delete variable {variable}, "
    ...
    def append_operation(self, operation: Operation):
        ...
        if not all([var.name in self.variables for var in operation.inputs + operation.outputs]):
            raise KeyError(
                f"Inserting Operation {operation} has a related variable "
    ...
    def topological_sort(self) -> List[Operation]:
        ...
        num_of_inputs = {
            operation.name: len(self.get_upstream_operations(operation)) for operation in self.operations.values()
        }
    ...
    def insert_op_between_ops(self, inserting_op: Operation, up_op: Operation, down_op: Operation):
        ...
        if down_op not in self.get_downstream_operations(up_op):
            raise PermissionError(
                f"operation {up_op.name} and {down_op.name} are not linked,"
        ...
        assert len(variables) == 1, (
            f"Can not insert operation between {up_op.name} and {down_op.name}," " graph is too complex."
        )
        [variable] = variables
        ...
        # create all links.
        link_var = self.create_variable(
            name=None, value=None, is_parameter=False, dest_ops=[down_op], source_op=inserting_op
        )
    ...
    def insert_op_between_var_and_op(self, inserting_op: Operation, up_var: Variable, down_op: Operation):
        ...
        if down_op.name not in [op.name for op in up_var.dest_ops]:
            raise PermissionError(
                f"variable {up_var.name} and {down_op.name} are not linked,"
                " there is no way to insert an op between them."
            )
        if len(inserting_op.inputs) != 0 or len(inserting_op.outputs) != 0:
            raise PermissionError("Can only insert operation with no input and output variables.")
        ...
        for var in down_op.inputs:
            if var == up_var:
                variables.append(var)
        assert len(variables) == 1, (
            f"Can not insert operation between {var.name} and {down_op.name}," " graph is too complex."
        )
    ...
    def create_link_with_op(self, variable: Variable, upstream_op: Operation, downstream_op: Operation):
        ...
        if variable.source_op != upstream_op:
            raise PermissionError(
                f"Can not create link with variable {variable}, " f"cause its source operations != {upstream_op}"
            )
        ...
        if downstream_op is not None and variable not in downstream_op.inputs:
            ...
            ppq_warning(
                f"You are trying to link variable with operation, "
    ...
    def create_operation(
        ...
        **kwargs,
        ...
        if not isinstance(inputs, list):
            raise TypeError(
                f"A list of input variable is required for creating operation, " f"however {type(inputs)} was given"
            )

        if attributes is None:
            attributes = {}
        created = Operation(
            name=name, op_type=op_type, attributes=attributes, platform=platform, inputs=inputs, outputs=outputs
        )
        ...
        if not isinstance(outputs, list):
            raise TypeError(
                f"A list of output variable is required for creating operation, " f"however {type(inputs)} was given"
            )
        for item in outputs:
            if not isinstance(item, Variable):
                raise TypeError(
