...
class GraphReplacer(GraphCommandProcessor):
    def process(self, command: GraphCommand) -> Any:
        if command.command_type == GraphCommandType.REPLACE_OP:
            assert isinstance(command, ReplaceOperationCommand), "Use ReplaceOperationCommand instead of GraphCommand"
            return self.replace_op(command.op_name, command.replace_to)
        if command.command_type == GraphCommandType.REPLACE_VAR:
            assert isinstance(command, ReplaceVariableCommand), "Use ReplaceOperationCommand instead of GraphCommand"
...
class GraphFormatter(GraphCommandProcessor):
    ...
    def format_slice(self) -> None:
        ...
        for slice in interested_ops:
            ...
            if "axes" in slice.attributes:
                slice.attributes.pop("axes")
    ...
    def format_pad(self) -> None:
        ...
        for operation in interested_ops:
            ...
            if padding_mode == "constant":
                operation.attributes["pads_value"] = padding_value
    ...
    def __delete_constant_input(self, op: Operation, input_idx: int):
        ...
        operation = self._graph.operations[op_name]
        assert input_idx < len(operation.inputs), (
        ...
        if input_var.source_op.type != "Constant":
            raise ValueError(
                f"Trying to delete an non-const input, " f"Error at Operation {op_name}, inputs[{input_idx}]"
            )
...
class GraphMerger(GraphCommandProcessor):
    ...
    def fuse_bn(self):
        ...
        for path in paths:
            ...
            if computing_op.type == "Conv":
                ...
                raise TypeError(
                    f"Unexpected op type {computing_op.type}. " f"Can not merge {computing_op.name} with {bn_op.name}"
                )
    ...
    def fuse_gemm(self):
        """Fuse MatMul + add into a singal Gemm Single Matmul will be replaced
        with Gemm.
    ...
    def decompose_gemm(self):
        ...
        for op in interested_ops:
            ...
            if op.num_of_input == 3:
                ...
                graph.create_link_with_op(variable=graph.create_variable(), upstream_op=op, downstream_op=bias_add)
...
class GraphDeviceSwitcher(GraphCommandProcessor):
    ...
    def insert_switcher(self):
        ...
        def can_pass_shape(from_op: Operation, to_op: Operation) -> bool:
            if to_op.platform == TargetPlatform.SHAPE_OR_INDEX:
                return True
            else:
                return not value_tracing_pattern(from_where=from_op, to_where=to_op)
        ...
        for operation in soi_ops:
            ...
            for var in operation.outputs:
                if all([can_pass_shape(operation, op) for op in var.dest_ops]):
                    continue
                ...
                if all([not can_pass_shape(operation, op) for op in var.dest_ops]):
                    ...
                    for dest_op in var.dest_ops:
                        if can_pass_shape(operation, dest_op):
                            continue
                        boundary_op = self.graph.create_operation(
                            op_type="PPQDeviceSwitch", platform=TargetPlatform.FP32
                        )
            ...
            for var in operation.inputs:
                ...
                if source_op is None and var.name in self.graph.inputs:
                    boundary_op = self.graph.create_operation(
                        op_type="PPQDeviceSwitch", platform=TargetPlatform.SHAPE_OR_INDEX
                    )
    ...
    def _acceptable_command_types(self) -> List[GraphCommandType]:
        return [GraphCommandType.INSERT_SWITCHER, GraphCommandType.REMOVE_SWITCHER]
