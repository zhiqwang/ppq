...
from ppq.IR import BaseGraph, BaseGraph, GraphFormatter, Operation, QuantableOperation, Variable
...
class HorizontalLayerSplitPass(QuantizationOptimizationPass):
    ...
    def __init__(
        ...
        if self.interested_layers is None or len(self.interested_layers) == 0:
            raise ValueError(
                "Layer Split Pass(Lateral) Requires a list of spliting layers, "
    ...
    def h_split(self, op: Operation) -> Tuple[torch.Tensor, torch.Tensor, int]:
        ...
        if self.verbose:
            print("")
            print(f"# Layer {op.name} has been splited, " f"{processed_values}/{value.numel()} value(s) was processed.")
        ...
        with torch.no_grad():
            for name in self.interested_layers:
                ...
                if isinstance(op1, QuantableOperation):
                    raise TypeError(
                        f"Can not split a quantized operation, "
                ...
                if processed_values > 0:
                    ...
                    op2 = graph.create_operation(
                        op_type=op1.type, attributes=op1.attributes.copy(), platform=op1.platform
                    )
                    input_var, output_var = op1.inputs[0], op1.outputs[0]
                    graph.create_link_with_op(
                        variable=op1.inputs[0], upstream_op=input_var.source_op, downstream_op=op2
                    )
                    ...
                    graph.create_link_with_op(variable=graph.create_variable(), upstream_op=op1, downstream_op=adder)
                    graph.create_link_with_op(variable=graph.create_variable(), upstream_op=op2, downstream_op=adder)
...
class GRUSplitPass(QuantizationOptimizationPass):
    ...
    def optimize(self, graph: BaseGraph, dataloader: Iterable, executor: BaseGraphExecutor, **kwargs) -> None:
        ...
        for op in interested_ops:
            ...
            graph.create_link_with_op(
                variable=graph.create_variable(value=torch.tensor([0]), is_parameter=True),
                upstream_op=None,
                downstream_op=op5,
            )
            graph.create_link_with_op(
                variable=graph.create_variable(value=torch.tensor([hidden_size]), is_parameter=True),
                upstream_op=None,
                downstream_op=op5,
            )
            graph.create_link_with_op(
                variable=graph.create_variable(value=torch.tensor([1]), is_parameter=True),
                upstream_op=None,
                downstream_op=op5,
            )
            graph.create_link_with_op(
            ...
            graph.create_link_with_op(
                variable=graph.create_variable(value=torch.tensor([2 * hidden_size]), is_parameter=True),
                upstream_op=None,
                downstream_op=op6,
            )
            graph.create_link_with_op(
