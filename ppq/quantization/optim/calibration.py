    ...
    def optimize(
        ...
        **kwargs,
        ...
        assert calib_steps <= 512, (
            "Calibration steps is too large, ppq is capable for quantizing your network within 8-512 "
        ...
        for op_name, operation in graph.operations.items():
            ...
            observer = OperationObserver(operation=executor._graph.operations[op_name], monitor_parameter=False)
        ...
        for op_name, observer in self._observers.items():
            ...
            if all(
                [
                    type(var_observer) not in {TorchHistObserver, TorchMSEObserver}
...
class PPLDSPTIReCalibrationPass(RuntimeCalibrationPass):
    ...
    def optimize(
        ...
        **kwargs,
        ...
        hooks = {}
        for operation in tqdm(graph.topological_sort(), desc="Collecting Observer For Computing Ops"):

            if not isinstance(operation, QuantableOperation) or not operation.is_computing_op:
                continue
            ...
            if operation.inputs[0].name in graph.inputs:
                ...
                observe_table.update(
                    {input_cfg: TensorObserverFactroy.build_observer(operation.inputs[0], sym_input_cfg)}
                )

            # only consider overlapped by relu/clip activation here
            downstream_ops = graph.get_downstream_operations(operation)
            if (
                len(downstream_ops) == 1
                and downstream_ops[0].type in {"Relu", "Clip"}
        ...
        self.calibrate(desc="ReCalibration For Computing Ops", dataloader=dataloader, executor=executor, hooks=hooks)
        ...
        for hook in hooks.values():
            ...
            for observer in hook._observer_table.values():
                ...
                if observer._quant_cfg.policy.has_property(QuantizationProperty.PER_CHANNEL):
                    min_vals = (
                        torch.min(torch.cat(observer._min_val_collector, dim=-1), dim=-1, keepdim=False)[0]
                        .cpu()
                        .numpy()
                    )
                    max_vals = (
                        torch.max(torch.cat(observer._max_val_collector, dim=-1), dim=-1, keepdim=False)[0]
                        .cpu()
                        .numpy()
                    )
