...
class QuantizationOptimizationPass(metaclass=ABCMeta):
    ...
    def apply(self, graph: BaseGraph, dataloader: Iterable, executor: BaseGraphExecutor, **kwargs) -> None:
        if not isinstance(graph, BaseGraph):
            raise TypeError(
                f"Incorrect graph object input, expect PPQ BaseGraph here, " f"while {type(graph)} was given."
            )
...
class QuantizationOptimizationPipeline(Container, Iterable):
    ...
    def optimize(
        self, graph: BaseGraph, dataloader: Iterable, executor: BaseGraphExecutor, verbose: bool = False, **kwargs
    ) -> None:
        max_name_length = 0
        ...
        for optim_pass in self._pipeline:
            if not isinstance(optim_pass, QuantizationOptimizationPass):
                raise TypeError(
                    f"Quantization Optimization Pipeline object only suppose to contain optimization passes only, "
            ...
            if verbose:
                ...
                print(
                    f'[{time.strftime("%H:%M:%S", time.localtime())}] {optim_pass.name} Running ... '
                    + " " * padding_length,
                    end="",
                )
            ...
            if not isinstance(graph, BaseGraph):
                raise TypeError(
                    f"parameter 1 should be an instance of PPQ BaseGraph when calling optim pass, "
