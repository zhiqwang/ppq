...
class LearningToCalibPass(TrainingBasedPass):
    ...
    def __init__(self, method: str = "e-greedy", calib_act: bool = True, calib_weight: bool = True) -> None:
        self.method = method
        ...
        with tqdm(total=self.target_step) as t:
            while cur_iter < self.target_step:
                ...
                qt_output = executor.partial_graph_forward(
                    operations=block.rps, feed_dict={input_var.name: qt_input}, output_names=[output_var.name]
                )[0]

                loss = torch_snr_error(y_pred=qt_output, y_real=fp_output).item()
                for delegator in delegators:
                    delegator.mark(1 - loss)
                loss_ema.push(loss)
    ...
    def collect_training_data(
        self, output_name: str, dataloader: Iterable, executor: BaseGraphExecutor, collate_fn: Callable
    ...
    def optimize(
        ...
        for op in graph.operations.values():
            ...
            if all([rp.is_computing_op == False for rp in block.rps]):
                continue
            if block.sp.is_computing_op == False:
                continue
        ...
        self.initialize_checkpoints(graph=graph, executor=executor, dataloader=dataloader, collate_fn=collate_fn)
        ...
        for bidx, block in enumerate(blocks):
            ...
            fp32_outputs = self.collect_training_data(
                output_name=block_output.name, dataloader=dataloader, executor=executor, collate_fn=collate_fn
            )
            ...
            quant_inputs = self.collect_training_data(
                output_name=block_input.name, dataloader=dataloader, executor=executor, collate_fn=collate_fn
            )
...
class MatrixFactorizationPass(QuantizationOptimizationPass):
    ...
    def __init__(self, interested_layers: List[str], method: str = "training", name: str = "SVD Split Pass") -> None:
    ...
    def train_for_factorization(
        self, w: torch.Tensor, penalty=0.1, executing_device: str = "cuda", max_iter: int = 100000
    ):
        assert w.ndim == 2
    ...
    def optimize(self, graph: BaseGraph, dataloader: Iterable, executor: BaseGraphExecutor, **kwargs) -> None:
        ...
        for operation in graph.operations.values():
            if operation.name in self.interested_layers:
                assert operation.type in {"Conv", "Gemm"}, f"Can not split layer, cause layer type is not support"
        ...
        for operation in splitting_layers:
            ...
            if operation.type == "Gemm":
                ...
                if self.method == "svd":
                    ...
                    raise ValueError(f"Invalid method {self.method}, only support training and svd now.")
                a = a.transpose(0, 1)
                b = b.transpose(0, 1)
                ...
                raise TypeError(f"Unsupported operation type {operation.type}.")
            ...
            graph.insert_op_on_var(inserting_op=splitted, var=operation.outputs[0].name)
            ...
            # add weight link
            spilted_w = Variable(name=splitted.name + "_weight", value=b, is_parameter=True)
