...
import torch
from ppq.core import ppq_warning, QuantizationProperty, QuantizationStates
        ...
        for op in graph.operations.values():
            ...
            if op.type in {"Conv", "ConvTranspose", "Gemm"}:
                ...
                if op.num_of_input == 3:
                    ...
                    if bias is None:
                        raise ValueError(
                            f"Bias Varaible {op.inputs[-1].name} must be constant. " "Please check it again."
                        )
                    ...
                    if self._override and (b_cfg.state == QuantizationStates.PASSIVE):
                        if not check_state(w_cfg.state):
                            raise PermissionError(
                                f"Can not quantize bias of layer {op.name}, "
    ...
    def optimize(self, graph: BaseGraph, dataloader: Iterable, executor: BaseGraphExecutor, **kwargs) -> None:
        ...
        for op_name, operation in graph.operations.items():
            ...
            observer = OperationObserver(
                operation=executor._graph.operations[op_name], monitor_outputs=False, monitor_inputs=False
            )
            observers[op_name] = observer
            hooks[op_name] = observer.hook

        # dummy forward, quant all parameter.
        assert isinstance(executor, TorchExecutor), "ParameterQuantizePass Only support TorchExecutor now."
