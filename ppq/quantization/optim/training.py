...
class TrainingBasedPass(QuantizationOptimizationPass):
    ...
    def split_graph_into_blocks(
        ...
        `block_limit` is not specified, block grandularity will be controlled
        by the default value OPTIM_ADVOPT_GRAPH_MAXSIZE specified in
        ppq.core.common.
    ...
    def collect(
        ...
        """Collect training data for given block. This function will collect
        fp32 output and quantized input data by executing your graph twice. For
        collecting fp32 output, all related operations will be dequantized. For
        collecting quantized input, all related operations' quantization state
        will be restored.
    ...
    def compute_block_loss(
        ...
        """loss computing for fp32 and quantized graph outputs, used in
        multiple training-based algorithms below.
...
class BiasCorrectionPass(TrainingBasedPass):
    ...
    * loss_fn(Callable)
    ...
    def __init__(
        self, interested_layers: List[str] = [], collecting_device: str = "cuda", steps: int = 32, block_size: int = 1
    ) -> None:
        ...
        with torch.no_grad():
            ...
            for qt_input, _ in tqdm(zip(qt_inputs, fp_outputs), "# Bias Correction Phrase 1"):
                ...
                outputs = executor.partial_graph_forward(
                    operations=block.rps, feed_dict=feed_dict, output_names=interested_outputs
                )
    ...
    def optimize(
        ...
        for block_idx, block in enumerate(blocks):
            ...
            pre_loss, post_loss = self.correct_bias(
                qt_inputs=qt_inputs, fp_outputs=fp_outputs, block=block, executor=executor, graph=graph
            )
