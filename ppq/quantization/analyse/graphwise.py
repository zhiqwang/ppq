...
import torch
from ppq.core import OperationMeta, PASSIVE_OPERATIONS, ppq_warning
...
class OutputRecorder(RuntimeHook):
    def __init__(self, operation: Operation, operation_meta: OperationMeta = None, fetchs: int = 4096) -> None:
        self.fetched = None
        self.fetchs = fetchs
    ...
    def post_forward_hook(self, outputs: list, **kwargs) -> list:
        ...
        self.fetched = batch_random_fetch(output_tensor, seed=10086, fetchs_per_batch=self.fetchs).to("cpu")
...
class DetailedRecorder(RuntimeHook):
    def __init__(self, operation: Operation, operation_meta: OperationMeta = None, fetchs: int = 1024) -> None:
        self.fetchs = fetchs
...
def statistical_analyse(
    ...
    for operation in interested_op:
        if isinstance(operation, QuantableOperation):
            hooks[operation.name] = DetailedRecorder(operation=operation, operation_meta=operation.meta_data)
            caches[operation.name] = {
                "Quantized Input": [],
                "Quantized Output": [],
                "Dequantized Input": [],
