...
class MetaxTensorwiseQuantizer(BaseQuantizer):
    ...
    def build_prequant_pipeline(
        ...
        return (
            super()
            .build_prequant_pipeline(setting, executor)
            .append_optimization_to_pipeline(MetaxGemmSplitPass(), at_front=True)
        )
        ...
        if operation.type in {"Conv", "MatMul"}:
            ...
            # layout: [in_dim, out_dim]
                ...
                base_quant_config.input_quantization_config[
                    ...
                    convert_from=matmul_weight_config, offset=None, scale=None, channel_axis=1
                )
                base_quant_config.input_quantization_config[1].observer_algorithm = "Minmax"
            # if operation has bias
