...
from ppq.IR import BaseGraph, GraphReplacer, Operation, QuantableGraph, QuantableOperation, QuantableVariable
...
class BaseQuantizer(metaclass=ABCMeta):
    def __init__(self, graph: BaseGraph, verbose: bool = True) -> None:
        if not isinstance(graph, BaseGraph):
            raise TypeError(
                f"To initialize a Quantizer, a BaseGraph instance is needed."
    ...
    def quantize(
        ...
        **kwargs,
        ...
        prequant_pipeline = self.build_prequant_pipeline(setting, executor=executor)
    ...
    def quantize_operations(
        ...
        if operation_platforms is None:
            operation_platforms = {}
        if operation_quantization_configs is None:
            operation_quantization_configs = {}
        ...
        for op_name, operation in self._graph.operations.items():
            ...
            if operation.platform != TargetPlatform.UNSPECIFIED:
                ...
                operation_platforms[op_name] = self.default_platform
        ...
        for op_name, operation in self._graph.operations.items():
            if not TargetPlatform.is_quantized_platform(operation_platforms[op_name]):
                continue
            ...
            if operation.type in quantable_operation_types or TargetPlatform.is_quantized_platform(operation.platform):
                ...
                if op_name in operation_quantization_configs:
                    continue
                else:
                    operation_quantization_configs[op_name] = self.init_quantize_config(operation=operation)
        ...
        for op_name, operation in list(self._graph.operations.items()):
            ...
            if TargetPlatform.is_quantized_platform(target_platform):
                if op_name not in operation_quantization_configs:
                    raise KeyError(
                        f"Can not find quantization configuration for operation {op_name}, "
                ...
                quantize_chain(
                    QuantizeOperationCommand(
                        op_name=operation.name, target_platform=target_platform, config=quantization_config
                    )
                )
            else:
    ...
    def build_quant_pipeline(self, setting: QuantizationSetting) -> QuantizationOptimizationPipeline:
        ...
        if setting.advanced_optimization == True:
            ...
            ppq_warning(
                "PPQ Advanced optimization 在 0.6.5 版本中已经被移除且不会起到任何效果，作为替代方案我们建议你使用 setting.lsq_optimization = True"
            )
