...
from ppq.core import ppq_warning, TargetPlatform
from ppq.IR import BaseGraph, Operation, SearchableGraph

from .base import GraphDispatcher
from .core import DEFAULT_SOCKET_CREATOR, DEFAULT_SOCKET_TABLE, OpSocket, VProperty, VType
...
class Perseus(GraphDispatcher):
    ...
    * ---------------------- *
    ...
    def __init__(self, graph: BaseGraph, verbose: bool = False) -> None:
        """初始化英仙座调度器 在这一过程中，英仙座调度器将为每一个算子生成它的 OpSocket 接线器
        ...
        for op in self.graph.operations.values():
            if op.type in DEFAULT_SOCKET_TABLE:
                ...
                ppq_warning(
                    f"Perseus Dispatcher do not konw how to dispatch op {op.name}({op.type}), "
    ...
    def solve_transitive_closure(self, sources: List[Operation], recursive: bool = True) -> Set[Operation]:
        ...
        """
        if isinstance(sources, Operation):
            sources = [sources]
    ...
    def mark_non_quantable_op(self) -> Set[Operation]:
        """追踪图中所有不可量化节点，即求解所有不可量化节点的传递闭包 不可量化节点种类繁多，常见的节点包括 Shape, Topk, NMS
        有一些节点的输入不能量化，如 Reshape, Slice 等 所有不可量化节点由 opsocket 具体定义给出.
    ...
    def parse_transitive_fanout(self, parsing_from: List[Operation]) -> Set[Operation]:
        """从指定节点出发，寻找所有扇出节点 注意此函数与 graph.get_downstream_operations 的区别
    ...
    def parse_transitive_fanin(self, parsing_from: List[Operation]) -> Set[Operation]:
        """从指定节点出发，寻找所有扇出节点 注意此函数与 graph.get_upstream_operations 的区别
    ...
    def dispatch(
        self, graph_input_cls: List[VProperty] = None, graph_output_cls: List[VProperty] = None
