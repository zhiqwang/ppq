...
class VType:
    ...
    def non_quantable(self) -> bool:
        return (
            self.has_property(VProperty.ATTRIB)
            or self.has_property(VProperty.LOGICAL)
            or self.has_property(VProperty.SOI)
        )


class VLink:
    def __init__(self, input_idx: int, output_idx: int, direction: str = "Bidirectional") -> None:
        if not isinstance(input_idx, int):
            raise TypeError(f"Can not create vlink with input_idx {input_idx}, " "only int value is acceptable here.")
        if not isinstance(output_idx, int):
            raise TypeError(f"Can not create vlink with output_idx {output_idx}, " "only int value is acceptable here.")

        if direction.lower() not in {"bidirectional", "forward", "backwrad"}:
            raise KeyError(
                f"Can not create vlink with direction {direction}, "
...
class OpSocket:
    def __init__(
        self,
        op: Operation,
        cls_input: List[VProperty] = None,
        cls_output: List[VProperty] = None,
    ...
    def set_input_cls(self, idx: int, cls: VProperty):
        if idx >= len(self.cls_input) or idx < 0:
            raise IndexError(
                "Opsocket Can not initilized. Input index out of range. "
                f"Except index between 0 and {len(self.cls_input) - 1}, while {idx} was given."
            )
        if not isinstance(cls, VProperty):
            raise TypeError(
                f"Opsocket Can not initilized. Can not set VClass for op {self.op.name}, "
