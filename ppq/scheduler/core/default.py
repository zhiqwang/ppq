...
from ppq.core import ONNX_DOMAIN, ppq_warning, STRICT_OPSET_CHECKING
from ppq.IR import Operation

from .opsocket import OpSocket, VLink, VProperty
...
def CHECK_OPSET(
    min_version_supported: int, max_version_supported: int, op: Operation, strict_check: bool = STRICT_OPSET_CHECKING
):
    opset = op.opset
...
def Slice_Socket(op: Operation) -> OpSocket:
    ...
    return OpSocket(
        op=op,
        cls_input=[VProperty.VALUE] + [VProperty.SOI for _ in range(op.num_of_input - 1)],
...
def Pad_Socket(op: Operation) -> OpSocket:
    ...
    return OpSocket(op=op, cls_input=cls_input[: op.num_of_input], links=[VLink(input_idx=0, output_idx=0)])
...
def Topk_Socket(op: Operation) -> OpSocket:
    ...
    return OpSocket(
        op=op, cls_input=cls_input[: op.num_of_input], cls_output=cls_output, links=[VLink(input_idx=0, output_idx=0)]
    )
...
def ConstantOfShape_Socket(op: Operation) -> OpSocket:
    ...
    return OpSocket(op=op, cls_input=cls_input[: op.num_of_input], links=[])
...
def NonZero_Socket(op: Operation) -> OpSocket:
    ...
    return OpSocket(op=op, cls_input=cls_input[: op.num_of_input], cls_output=cls_output, links=[])
...
def ScatterElements_Socket(op: Operation) -> OpSocket:
    ...
    return OpSocket(
        op=op,
        cls_input=cls_input[: op.num_of_input],
