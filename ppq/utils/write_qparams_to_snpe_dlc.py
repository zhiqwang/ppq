...
def write_qparams_to_dlc_model(input_dlc, output_dlc, activation_qparams):
    ...
    for snpe_layer in model.get_layers():
        ...
        for snpe_layer_out_ind, snpe_layer_out in enumerate(snpe_layer["output_names"]):
            ...
            print(
                "original quant encodings : ",
                model.get_tf_output_encoding_by_index(name=layer_name, index=snpe_layer_out_ind),
            )
            ...
            model.set_tf_output_encoding_by_index(
                name=layer_name, index=snpe_layer_out_ind, bitwidth=8, min=new_enc["min"], max=new_enc["max"]
            )
            print(
                "ppq quant encodings : ",
                model.get_tf_output_encoding_by_index(name=layer_name, index=snpe_layer_out_ind),
