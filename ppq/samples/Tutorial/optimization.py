...
class MyOptim(QuantizationOptimizationPass):
    def optimize(
        self, graph: BaseGraph, dataloader: Iterable, collate_fn: Callable, executor: TorchExecutor, **kwargs
    ) -> None:
...
with ENABLE_CUDA_KERNEL():
    ...
    optim.optimize(
        ...
        collate_fn=INPUT_SHAPE,
        executor=TorchExecutor(quantized, device=DEVICE),
    )
