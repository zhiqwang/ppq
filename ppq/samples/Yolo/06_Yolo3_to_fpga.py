...
class MyFPGAQuantizer(FPGAQuantizer):
    """不同厂商的FPGA量化规则不一样， 你需要自定义量化器，根据硬件执行规则调整量化方案."""
...
with ENABLE_CUDA_KERNEL():
    ...
    graphwise_error_analyse(
        graph=ppq_graph_ir, running_device="cuda", dataloader=dataloader, collate_fn=lambda x: x.to("cuda")
    )

    export_ppq_graph(graph=ppq_graph_ir, platform=TargetPlatform.ONNXRUNTIME, graph_save_to="Quantized.onnx")
