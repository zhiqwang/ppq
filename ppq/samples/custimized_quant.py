...
def generate_samples(num_of_samples: int = 32):
    ...
    sample = {"speech": torch.rand(size=[B, T, 80]).float().cuda(), "speech_lengths": torch.ones(size=[B]).int().cuda()}
...
register_operation_handler(
    handler=layernorm_forward, operation_type="LayerNormPlugin", platform=TargetPlatform.TRT_INT8
)

register_network_quantizer(quantizer=MyTensorRTQuantizer, platform=TargetPlatform.TRT_INT8)
...
with ENABLE_CUDA_KERNEL():
    ...
    graphwise_error_analyse(graph=qir, running_device="cuda", dataloader=SAMPLES)

    export_ppq_graph(graph=qir, platform=TargetPlatform.ONNXRUNTIME, graph_save_to="quantized.onnx")
