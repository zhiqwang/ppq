...
with ENABLE_CUDA_KERNEL():
    ...
    graphwise_error_analyse(
        graph=quantized, running_device="cuda", dataloader=SAMPLES, collate_fn=lambda x: x.cuda(), steps=32
    )

    export_ppq_graph(graph=quantized, platform=TargetPlatform.OPENVINO_INT8, graph_save_to="model_int8.onnx")
...
model = openvino_executor.compile_model(model=openvino_executor.read_model(model="model_int8.onnx"), device_name="CPU")
tick = time.time()
