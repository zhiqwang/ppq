...
class ORTOOSExporter(ONNXRUNTIMExporter):
    ...
    def convert_operation(
        ...
        process_activation: bool,
        process_parameter: bool,
        ...
        if op.type in self.ONNX_QUANTABLE_TABLE:
            ...
            for config, var, meta in zip(op.config.input_quantization_config, inputs, input_metas):
                ...
                graph.create_link_with_op(
                    variable=graph.create_variable(value=scale, is_parameter=True), upstream_op=None, downstream_op=op
                )
                graph.create_link_with_op(
                    variable=graph.create_variable(value=offset, is_parameter=True), upstream_op=None, downstream_op=op
                )
            ...
            op.meta_data.output_metas = [
                TensorMeta(DataType.convert_from_torch(vtype), shape=op.meta_data.output_metas[0].shape)
            ]

            # process bias
            if bias is not None:
                assert isinstance(
                    bias_config, TensorQuantizationConfig
                ), "Unexpected bias quantization configuration type."
                if bias_config.state not in {
                    QuantizationStates.ACTIVATED,
                    QuantizationStates.PASSIVE,
                    QuantizationStates.BAKED,
                    QuantizationStates.PASSIVE_BAKED,
                ...
                if bias_config.num_of_bits <= 16:
                    ppq_warning(
                        f"Bias vector of operation {op.name} is quantized with {bias_config.num_of_bits} bits, "
    ...
    def prepare_graph(
        ...
        super().prepare_graph(
            graph, process_activation, process_parameter, remove_activation_fn, quant_parameter_to_int
        )
        FP32_ONLY_TYPES = {
            "Add",
            "Mul",
        ...
        quantize_extension = processor.opset_matching(
            ...
            ep_expr=lambda x: x.type in FP32_ONLY_TYPES
            or (not TargetPlatform.is_quantized_platform(x.platform))
            or x.is_boundary,
        )
        ...
        for op in [op for op in quantize_extension]:
            if op.type in FP32_ONLY_TYPES:
                ...
                self.insert_dequant_on_variable(graph=graph, var=op.inputs[0], config=qconfig, related_op=op)
        ...
        for op in quantize_extension:
            for input_var in op.inputs:
                if input_var.is_parameter:
                    continue
                if op.type not in {"QGemm", "QLinearConv", "QLinearMatMul"}:
                    continue
                if input_var.source_op not in quantize_extension or input_var.source_op is None:
                    ...
                    self.insert_quant_on_variable(graph=graph, var=input_var, config=qconfig, related_op=op)
        ...
        for output_var in graph.outputs.values():
            if output_var.source_op in quantize_extension:
                ...
                self.insert_dequant_on_variable(config=qconfig, graph=graph, var=output_var, related_op=op, meta=meta)
                created = op.inputs[0]
