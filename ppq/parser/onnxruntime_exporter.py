...
class QDQHelper:
    ...
    def TQC_Exportable_Check(TQC: TensorQuantizationConfig, bounded_var: Variable) -> bool:
        ...
        if not meta_check:
            raise ValueError(
                f"Meta Data is missing! Graph Export Failed. " f"(Check Meta For Varaible: {bounded_var.name})"
            )
        return True
...
class ONNXRUNTIMExporter(OnnxExporter):
    ...
    def insert_quant_on_variable(
        ...
        if related_op is not None and var in related_op.inputs:
            ...
            graph.insert_op_on_var(created, var=var.name)
        ...
        meta = OperationMeta(
            ...
            operation_name=created.name,
            operation_type=created.type,
            executing_order=-1,
        )
    ...
    def insert_dequant_on_variable(
        ...
        dq_meta = OperationMeta(
            input_metas=[
                TensorMeta(dtype=DataType.convert_from_torch(value_dtype), shape=meta.shape),
                TensorMeta(dtype=DataType.FP32, shape=config.scale.shape),
                TensorMeta(dtype=DataType.convert_from_torch(offset_dtype), shape=config.offset.shape),
            ],
    ...
    def remove_activation_ops(self, graph: BaseGraph) -> BaseGraph:
        ...
        for op in removed_activations:
            ...
            self.insert_dequant_on_variable(
                graph=graph, var=input_var, config=quant_config, related_op=upstream_op, meta=input_var.meta
            )
    ...
    def remove_duplicated_quant_op(self, graph: BaseGraph) -> BaseGraph:
        ...
        for qt_op in graph.operations.values():
            if qt_op.type == "QuantizeLinear":
                if len(graph.get_upstream_operations(qt_op)) != 1:
                    continue
                if graph.get_upstream_operations(qt_op)[0].type != "DequantizeLinear":
                    continue
    ...
    def convert_operation(
        ...
        for config, var in op.config_with_variable:
            ...
            if var.is_parameter and process_parameter:
                ...
                self.insert_dequant_on_variable(graph=graph, var=var, config=config, related_op=op, meta=meta)
                ...
                created = self.insert_quant_on_variable(graph=graph, var=var, config=config, related_op=op, meta=meta)
                self.insert_dequant_on_variable(
                    graph=graph, var=created.outputs[0], config=config, related_op=op, meta=meta
                )
    ...
    def export(self, file_path: str, graph: BaseGraph, config_path: str = None) -> None:
        ...
        graph_def = helper.make_graph(
            name=name, nodes=_nodes, inputs=_inputs, outputs=_outputs, initializer=_initilizers
        )
