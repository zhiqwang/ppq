...
import numpy as np
import torch
from ppq.core import convert_any_to_numpy, DataType


def convert_value(
    value: Union[int, float, np.ndarray, torch.Tensor], export_as_float: bool, dtype: DataType = DataType.FP32
) -> Union[float, list]:
    ...
    if dtype not in {DataType.FP32, DataType.INT32}:
        raise ValueError(
            f"Can Only export dtype fp32 and int32, " f"while you are requiring to dump a {dtype.name} value"
        )
