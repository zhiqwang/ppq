...
class OnnxExporter(GraphExporter):
    ...
    def export_graph(self, graph: BaseGraph) -> onnx.GraphProto:
        """Convert a PPQ IR to Onnx IR. This export will only convert PPQ Op
        and var to onnx, all quantization configs will be skipped.

        This function will try to keep the opset version of your graph
        unchanged. However if the opset is not given, ppq will convert it to
        with the global parameter ppq.core.ONNX_EXPORT_OPSET.
        """
        name = graph._name
        if not name:
    ...
    def export_operation(self, operation: Operation) -> onnx.OperatorProto:
        """Convert PPQ Op to Onnx Operation An Op consumes zero or more
        Tensors, and produces zero or more Tensors."""
    ...
    def export_var(self, variable: Variable) -> onnx.TensorProto:
        """Convert PPQ Variable to Onnx Tensor, There are 2 different types of
        Tensor in Onnx:
