...
from ppq.core import DataType, NetworkFramework, QuantizationProperty, QuantizationStates
    ...
    def export_quantization_config(self, config_path: str, graph: BaseGraph):
        ...
        for op in topo_order:
            if op.is_computing_op and isinstance(op, QuantableOperation):
                ...
                if not param_cfg.can_export():
                    continue

                assert (
                    param_cfg.state in {QuantizationStates.BAKED, QuantizationStates.ACTIVATED}
                    and param_cfg.observer_algorithm in {"minmax", "Minmax"}
                    and param_cfg.policy.has_property(QuantizationProperty.PER_CHANNEL)
                )
