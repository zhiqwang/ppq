...
import numpy as np
import torch
from ppq.core import CAFFE_DOMAIN, empty_ppq_cache, IS_DISPATCHED_GRAPH, NetworkFramework
from ppq.executor import TorchExecutor
from ppq.IR import BaseGraph, GraphCommand, GraphCommandType, GraphFormatter, GraphMerger, Operation, Opset, Variable
...
def build_temp_graph(
    ...
    for node in nodes:
        ...
        graph.operations[node["name"]] = Operation(
            name=node["name"],
            op_type=node["op_type"],
            attributes=node["attribute"].copy(),
            opset=Opset(domain=CAFFE_DOMAIN, version=1),
        )
...
class CaffeOpBuilder(object):
    ...
    def __init__(self, graph: ppl_caffe_pb2.NetParameter, layer: ppl_caffe_pb2.LayerParameter, input_shape: List[list]):
        self.graph = graph
        self.layer = layer
        self.settings = {}
        self.initializer = {}
        ...
        self.nodes.append(
            {
                ...
                "inputs": list(self.layer.bottom) + param_name,
                "outputs": list(self.layer.top),
...
class Reshape(CaffeOpBuilder):
    def trans(self):
        ...
        self.nodes = [
            {
                "name": self.layer.name,
                "inputs": list(self.layer.bottom) + [init_name],
                "outputs": list(self.layer.top),
                "op_type": self.layer.type,
                "attribute": {},
            }
...
class InnerProduct(CaffeOpBuilder):
    ...
    def trans(self):
        ...
        self.nodes.append(
            {
                "name": self.layer.name,
                "inputs": input_var_list + param_name,
                "outputs": list(self.layer.top),
                "op_type": self.op_type,
                "attribute": self.get_attr(),
            }
...
class NNUpsample(CaffeOpBuilder):
    ...
    def trans(self):
        ...
        self.initializer = {
            roi: {"dims": [4], "value": np.array([0, 0, 0, 0], dtype="f")},
        ...
        self.nodes = [
            {
                "name": self.layer.name,
                "inputs": list(self.layer.bottom) + [roi, scales],
                "outputs": list(self.layer.top),
                "op_type": self.op_type,
...
class Slice(CaffeOpBuilder):
    def trans(self):
        ...
        for i, pairs in enumerate(splits):
            ...
            self.nodes.append(
                {
                    "name": self.layer.name + "_" + str(i),
                    "inputs": list(self.layer.bottom) + [starts, ends, axis_name],
                    "outputs": [list(self.layer.top)[i]],
                    "op_type": self.op_type,
                    "attribute": {},
                }
            )
