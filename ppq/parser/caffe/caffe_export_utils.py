import numpy as np
from ppq.core.data import convert_any_to_numpy, convert_any_to_python_primary_type, DataType
...
class Softmax(CaffeOpExporter):
    def set_attr(self):
        ...
        if not (axis == -1 or axis == len(self.op.inputs[0].meta.shape) - 1):
            logger.warning(
                f"Converting to caffe Softmax, the axis={axis}, which is not the last axis. "
        ...
        for var in self.op.parameters:
            ...
            if value.size != 1:
                raise AttributeError(
                    f"Now don't support Mul op with initializer in shape {value.shape} convert to caffe"
                )
...
class Reshape(CaffeOpExporter):
    def set_attr(self):
        if len(self.op.inputs) == 0:
            raise AttributeError(
                f"{self.op.name} has no inputs. Cannot convert to caffe op. " "Please optimize the onnx model."
            )
...
class Pad(CaffeOpExporter):
    def set_attr(self):
        ...
        pads = (
            convert_any_to_numpy(self.op.inputs[1].value)
            if len(self.op.inputs) > 1
            else refine_value(self.op.attributes["pads"])
        )
        if len(pads) == 2:
...
class ConvTranspose(CaffeOpExporter):
    def set_attr(self):
        ...
        self.layer.convolution_param.num_output = self.op.parameters[0].value.shape[1] * self.op.attributes.get(
            "group", 1
        )
        self.layer.convolution_param.group = refine_value(self.op.attributes.get("group", 1))
        self.layer.convolution_param.kernel_h = kernel_h
...
class Slice(CaffeOpExporter):
    def parse(self):
        ...
        starts, ends = convert_any_to_numpy(self.op.parameters[0].value), convert_any_to_numpy(
            self.op.parameters[1].value
        )
        axes = (
            convert_any_to_numpy(self.op.parameters[2].value)
            if len(self.op.parameters) >= 3
            else [i for i in range(len(input_shape))]
        )
