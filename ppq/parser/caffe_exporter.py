...
from .caffe.caffe_export_utils import caffe_export_map, CaffeOpExporter
...
def convert_type(platform: TargetPlatform) -> str:
    if platform == TargetPlatform.PPL_CUDA_INT8:
        return "INT8"
    if platform == TargetPlatform.PPL_DSP_INT8:
        return "INT8"
...
class CaffeExporter(GraphExporter):
    def export_quantization_config(self, config_path: str, graph: BaseGraph):
        ...
        for operation in graph.operations.values():
            ...
            for config, var in operation.config_with_variable:
                ...
                if config.state == QuantizationStates.SLAVE and var.name in var_quant_info_recorder:
                    continue
        ...
        for var in var_quant_info_recorder:
            ...
            var_quant_info_recorder[var] = {
                ...
                "tensor_min": convert_value(
                    config.scale * (config.quant_min - config.offset), tensorwise, DataType.FP32
                ),
                "tensor_max": convert_value(
                    config.scale * (config.quant_max - config.offset), tensorwise, DataType.FP32
                ),
        ...
        for op in graph.operations.values():
            if convert_type(op.platform) is not None:
                op_platform_recorder[op.name] = {"data_type": convert_type(op.platform)}

        exports = {"quant_info": var_quant_info_recorder, "op_info": op_platform_recorder}
    ...
    def prepare_model(self, graph: BaseGraph, input_shapes: List[List[int]]):
        ...
        if all([var.value is not None for var in graph.inputs.values()]):
            ...
            inputs = {
                var.name: torch.randn(*var.meta.shape, dtype=DataType.to_torch(var.meta.dtype), device=device)
    ...
    def dump_to_file(
        self, caffe_model: ppl_caffe_pb2.NetParameter, caffe_proto: ppl_caffe_pb2.NetParameter, file_path: str
    ):
    ...
    def export(
        ...
        self.dump_to_file(caffe_model=caffe_model, caffe_proto=caffe_proto, file_path=file_path)
...
class SNPECaffeExporter(CaffeExporter):
    ...
    def export(
        ...
        for idx in range(len(lines)):
            ...
            if "hole" in line or "ceil_mode" in line:
                continue
        ...
        for idx in range(len(caffe_proto.layer)):
            ...
            if record_bottom:
                for bottom in layer.bottom:
                    for (
                        cfg,
                        var,
                    ) in zip(op.config.input_quantization_config, op.inputs):
            ...
            for top in layer.top:
                for (
                    cfg,
                    var,
                ) in zip(op.config.output_quantization_config, op.outputs):
