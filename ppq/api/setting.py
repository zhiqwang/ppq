...
class EqualizationSetting:
    def __init__(self) -> None:
        ...
        self.value_threshold = 0.5  # try 0.5 and 2, it matters.
...
class QuantizationSettingFactory:
    ...
    def dsp_setting() -> QuantizationSetting:
        ...
        default_setting.equalization_setting.value_threshold = 0.5
    ...
    def from_json(json_obj: Union[str, dict]) -> QuantizationSetting:
        ...
        if "version" not in setting_dict:
            ...
            if version < PPQ_CONFIG.VERSION:
                ppq_warning(
                    f"You are loading a json quantization setting created by PPQ of another version: "
        ...
        if not isinstance(setting_dict, dict):
            raise TypeError(
                "Can not load setting from json string, "
