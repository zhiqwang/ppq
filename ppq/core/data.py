...
import numpy as np
import torch
from numpy import dtype as np_type, ndarray
from torch import dtype as torch_type, Tensor
...
class TensorMeta:
    def __init__(self, dtype: DataType, shape: List[int], tensor_name: str = None) -> None:
        ...
        if not isinstance(dtype, DataType):
            raise TypeError(
                f"Can not create Tensor Meta with dtype {type(dtype)}, "
    ...
    def parsing_from_torch_tensor(cls, torch_tensor: Tensor, name: str = None):
        if not isinstance(torch_tensor, Tensor):
            raise TypeError(
                f"Can not parse meta data for {type(torch_tensor)} instance, " "it should be torch.Tensor object."
            )
    ...
    def create_tensor(self, device: str, fill_value: Any = 0):
        return (
            torch.Tensor(size=self.shape, device="cpu")
            .fill_(fill_value)
            .type(dtype=DataType.to_torch(self.dtype))
            .to(device)
        )
...
class OperationMeta:
    ...
    def __str__(self) -> str:
        return "Inputs: ".join(str([_ for _ in self.input_metas])) + "Outputs: ".join(
            str([_ for _ in self.input_metas])
        )
...
def convert_any_to_python_primary_type(
    x: Union[torch.Tensor, np.ndarray, int, float, list, str], accept_none: bool = True
) -> Union[int, float, list, str]:
...
def convert_any_to_numpy(
    x: Union[torch.Tensor, np.ndarray, int, float, list, tuple], accept_none: bool = True
) -> np.ndarray:
    if x is None and accept_none:
        return None
    if x is None and not accept_none:
...
def convert_primary_type_to_list(x: Union[int, float, list, tuple]) -> list:
    if isinstance(x, list) or isinstance(x, tuple):
        return list(x)
    elif isinstance(x, int) or isinstance(x, float):
        return [x]
