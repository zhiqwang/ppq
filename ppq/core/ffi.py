...
class ComplieHelper(metaclass=SingletonMeta):
    ...
    def CUDA_EXTENSION(self):
        if self.__CUDA_EXTENTION__ is None:
            raise Exception(
                "Cuda Extension has not been compiled, " "invoke ppq.core.ffi.ComplieHelper.complie() First."
            )
...
class CUDA:
    ...
    @staticmethod
    def Histogram_T(
        tensor: torch.Tensor, histogram: torch.Tensor, scale: float, clip_outliers: bool = True
    ) -> torch.Tensor:
        # if scale < 1e-7: raise ValueError('scale is too small.')
    ...
    def Histogram_Asymmetric_T(
        min_value: float, max_value: float, tensor: torch.Tensor, histogram: torch.Tensor, clip_outliers: bool = True
    ) -> torch.Tensor:
        # if scale < 1e-7: raise ValueError('scale is too small.')
    ...
    @staticmethod
    def Histogram_C(
        tensor: torch.Tensor, channel_axis: int, histogram: torch.Tensor, scale: float, clip_outliers: bool = True
    ) -> torch.Tensor:
        # if scale < 1e-7: raise ValueError('scale is too small.')
