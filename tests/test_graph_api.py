...
graph.create_link_with_op(
    variable=graph.create_variable(), upstream_op=graph.operations["op1"], downstream_op=graph.operations["op2"]
)

processor = SearchableGraph(graph)
paths = processor.path_matching(
    sp_expr=lambda x: x.type == "Conv", rp_expr=lambda x, y: False, ep_expr=lambda x: x.type == "Conv", direction="down"
)
...
graph.create_link_with_op(
    variable=graph.create_variable(), upstream_op=graph.operations["op8"], downstream_op=graph.operations["op2"]
)
graph.create_link_with_op(
    variable=graph.create_variable(), upstream_op=graph.operations["op2"], downstream_op=graph.operations["op3"]
)
graph.create_link_with_op(
    variable=graph.create_variable(), upstream_op=graph.operations["op3"], downstream_op=graph.operations["op4"]
)
graph.create_link_with_op(
    variable=graph.create_variable(), upstream_op=graph.operations["op9"], downstream_op=graph.operations["op5"]
)
graph.create_link_with_op(
    variable=graph.create_variable(), upstream_op=graph.operations["op10"], downstream_op=graph.operations["op5"]
)
graph.create_link_with_op(
    variable=graph.create_variable(), upstream_op=graph.operations["op5"], downstream_op=graph.operations["op6"]
)
graph.create_link_with_op(
    variable=graph.create_variable(), upstream_op=graph.operations["op6"], downstream_op=graph.operations["op7"]
)
processor = SearchableGraph(graph)
paths = processor.path_matching(
...
paths = processor.path_matching(
    sp_expr=lambda x: x.type == "Conv", rp_expr=lambda x, y: True, ep_expr=lambda x: x.type == "Relu", direction="down"
)
...
paths = processor.path_matching(
    sp_expr=lambda x: x.type == "Conv", rp_expr=lambda x, y: False, ep_expr=lambda x: x.type == "Relu", direction="down"
)
assert len(paths) == 0
...
graph.create_link_with_op(
    variable=graph.create_variable(), upstream_op=graph.operations["op11"], downstream_op=graph.operations["op1"]
)
graph.create_link_with_op(
    variable=graph.create_variable(), upstream_op=graph.operations["op11"], downstream_op=graph.operations["op10"]
)
processor = GraphFormatter(graph)
